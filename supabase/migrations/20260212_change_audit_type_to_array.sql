-- 1. Modifier la colonne audit_type pour être un tableau de texte
ALTER TABLE public.cases 
ALTER COLUMN audit_type TYPE text[] 
USING CASE 
    WHEN audit_type IS NULL THEN NULL 
    ELSE ARRAY[audit_type] 
END;

-- 2. Mettre à jour la fonction RPC pour accepter un tableau pour audit_type
CREATE OR REPLACE FUNCTION public.create_case_and_debit(
  p_consultant_id uuid,
  p_tenant_name text,
  p_siret text,
  p_case_category text,
  p_audit_type text[], -- Changed from text to text[]
