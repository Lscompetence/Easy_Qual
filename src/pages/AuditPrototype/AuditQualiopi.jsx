...
export default function App() {
  const [org, setOrg] = useState({
    auditeurNom:"Marie Desnoyers", auditeurNum:"165", auditeurTel:"0663414253",
    auditeurMail:"contact@mariedesnoyers.fr",
    orgNom:"So forma", orgNum:"1679", orgVille:"75008 Paris", orgCA:"Entre 150 000 et 750 000 €",
    auditNum:"4416", dateDeb:"27.03.2023", dateFin:"27.03.2023", site:"Monosite",
    contactNom:"Charazed Bouali", contactMail:"charazed.bouali69@gmail.com", contactTel:"0783094069",
  ...
  // données par grille : { [gid]: { rep:{}, pts:{}, dec:{}, proc:{}, preuve:{} } }
    ...
    <div style={s.app}>
      ...
      <div style={s.shell}>
        ...
        <div style={s.config}>
          ...
          <div style={s.cfgGrid}>
            <F l="Auditeur"><input style={s.in} value={org.auditeurNom} onChange={e=>setOrg({...org,auditeurNom:e.target.value})}/></F>
            <F l="N° auditeur"><input style={s.in} value={org.auditeurNum} onChange={e=>setOrg({...org,auditeurNum:e.target.value})}/></F>
            <F l="Organisme audité"><input style={s.in} value={org.orgNom} onChange={e=>setOrg({...org,orgNom:e.target.value})}/></F>
            <F l="N° organisme"><input style={s.in} value={org.orgNum} onChange={e=>setOrg({...org,orgNum:e.target.value})}/></F>
            <F l="Ville"><input style={s.in} value={org.orgVille} onChange={e=>setOrg({...org,orgVille:e.target.value})}/></F>
            <F l="Tranche CA"><input style={s.in} value={org.orgCA} onChange={e=>setOrg({...org,orgCA:e.target.value})}/></F>
            <F l="N° audit"><input style={s.in} value={org.auditNum} onChange={e=>setOrg({...org,auditNum:e.target.value})}/></F>
            <F l="Date début"><input style={s.in} value={org.dateDeb} onChange={e=>setOrg({...org,dateDeb:e.target.value})}/></F>
            <F l="Date fin"><input style={s.in} value={org.dateFin} onChange={e=>setOrg({...org,dateFin:e.target.value})}/></F>
            <F l="Contact"><input style={s.in} value={org.contactNom} onChange={e=>setOrg({...org,contactNom:e.target.value})}/></F>
            <F l="Email contact"><input style={s.in} value={org.contactMail} onChange={e=>setOrg({...org,contactMail:e.target.value})}/></F>
            <F l="Site"><select style={s.in} value={org.site} onChange={e=>setOrg({...org,site:e.target.value})}><option>Monosite</option><option>Multisite</option></select></F>
          </div>
...
function Edit({ value, onChange, ph, small }) {
  return (
    <textarea value={value||""} onChange={e=>onChange(e.target.value)} placeholder={ph}
      style={{...s.edit, minHeight:small?44:64}}/>
  );
}
...
function F({ l, children }) {
  return <label style={{display:"block"}}><span style={s.fLbl}>{l}</span>{children}</label>;
}

/* ---- STYLES ---- */
const s = {
  app:{minHeight:"100vh",background:"#e9e9ec",fontFamily:"'Inter',-apple-system,system-ui,sans-serif",color:"#2a2a2a",padding:"20px 0 60px"},
  shell:{maxWidth:900,margin:"0 auto",padding:"0 16px"},
  config:{background:"#fff",borderRadius:12,padding:20,marginBottom:22,boxShadow:"0 1px 3px rgba(0,0,0,.06)"},
